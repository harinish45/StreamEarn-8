...
async function callEdge(action:string,payload:Record<string,unknown>={}){
  ...
  if(!response.ok) throw new Error(data?.error||`Supabase project operation failed (${response.status})`);
  return data;
}
...
export async function updateProject(id:string,patch:Record<string,unknown>){const data=await callEdge('update',{id,patch});return fromRow(data);}
