...
const items = [
  ...
  ['AI Browser','Reliable research launchpad','/browser',Globe2],
...
export default function UnifiedWorkspace() {
  ...
  useEffect(()=>{ try { setTasks(JSON.parse(localStorage.getItem('streamearn_tasks')||'[]')); } catch {} },[]);
  useEffect(()=>{ localStorage.setItem('streamearn_tasks', JSON.stringify(tasks)); },[tasks]);
  const filtered=useMemo(()=>items.filter(([name,desc])=>`${name} ${desc}`.toLowerCase().includes(q.toLowerCase())),[q]);
  const addTask=()=>{ if(task.trim()){setTasks(v=>[task.trim(),...v]);setTask('');} };
  return <SidebarProvider><UnifiedSidebar/><SidebarInset><Header showSidebarTrigger/><main className="min-h-[calc(100vh-4rem)] bg-background p-4 md:p-8"><div className="mx-auto max-w-7xl">
    <section className="rounded-3xl border bg-gradient-to-br from-primary/10 via-card to-card p-6 md:p-10"><div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-sm text-primary"><LayoutDashboard className="h-4 w-4"/> Unified Workspace</div><h1 className="mt-3 text-3xl font-bold md:text-5xl">Your StreamEarn command center.</h1><p className="mt-3 max-w-2xl text-muted-foreground">Navigate every part of the platform from one reliable workspace and keep a lightweight personal task list in your browser.</p></div><div className="relative w-full lg:w-96"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/><Input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search everything..." className="pl-9"/></div></div></section>
    ...
    <section className="mt-8 rounded-2xl border bg-card p-5"><div className="flex items-center gap-2 font-semibold"><ListTodo className="h-5 w-5 text-primary"/> Quick tasks</div><div className="mt-4 flex gap-2"><Input value={task} onChange={e=>setTask(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addTask()} placeholder="Add a task..."/><Button onClick={addTask}>Add</Button></div><div className="mt-4 space-y-2">{tasks.slice(0,8).map((t,i)=><button key={`${t}-${i}`} onClick={()=>setTasks(v=>v.filter((_,idx)=>idx!==i))} className="flex w-full items-center gap-3 rounded-lg border p-3 text-left text-sm hover:bg-muted"><CheckCircle2 className="h-4 w-4 text-primary"/>{t}</button>)}</div></section>
