...
function logoFor(url: string) { const domain = domainOf(url); return domain ? `https://www.google.com/s2/favicons?domain=${encodeURIComponent(domain)}&sz=128` : ''; }
...
function ToolVisual({ tool, category }: { tool: CurrentAITool; category: string }) {
  ...
  return <div className="relative h-28 overflow-hidden border-b bg-[#101114]">
    ...
    <div className="absolute bottom-3 left-4 flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border border-white/15 bg-white/95 shadow-lg">
      {!failed && <img src={logoFor(tool.url)} alt="" className="h-8 w-8 object-contain" onError={() => setFailed(true)} />}
      {failed && <span className="text-lg font-bold text-black/70">{tool.name.slice(0,1)}</span>}
    </div>
