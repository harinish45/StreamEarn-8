...
export function NewsSection({ compact = false }: { compact?: boolean }) {
  ...
  const items = useMemo(() => { const q=query.trim().toLowerCase(); const extra=(scheduledNews as ScheduledNews[]).filter(x=>!hidden.includes(x.id)).map(x=>({...x,image:imageForCategory(x.category),scheduled:true})); const base=aiNews.map(x=>({...x,scheduled:false})); return [...extra,...base].filter(item=>!q||[item.title,item.summary,item.category,item.source].join(' ').toLowerCase().includes(q)); }, [query,hidden]);
