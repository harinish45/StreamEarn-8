...
export function NewsSection({ compact = false }: { compact?: boolean }) {
  ...
  return <section className="space-y-6">
    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div><div className="flex items-center gap-2 text-primary"><Newspaper className="h-5 w-5" /><span className="text-xs font-semibold uppercase tracking-[0.2em]">AI Tech News</span></div><h2 className="mt-2 text-3xl font-semibold tracking-tight">What’s changing in AI right now</h2><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Curated updates focused on agents, developer tools, browser automation, infrastructure, security and AI products.</p></div>
