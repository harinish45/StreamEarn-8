  ...
  return (
    <Card className={`group overflow-hidden border-border/70 bg-card/80 transition duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg ${list ? 'flex' : ''}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <CardContent className="px-4 pb-4 pt-1 sm:px-5"><div className="mb-3 flex min-h-6 flex-wrap gap-1.5">{resource.tags.slice(0, 3).map((tag) => <Badge key={tag} variant="secondary" className="text-[10px] font-medium">{tag}</Badge>)}</div><Button asChild variant="outline" size="sm" className="h-9 w-full rounded-lg"><a href={resource.url} target="_blank" rel="noopener noreferrer">Open Platform <ExternalLink className="ml-2 h-3.5 w-3.5" /></a></Button></CardContent>
      </div>
    </Card>
  );
}
...
function ProjectCard({ project, index }: { project: FreeProject; index: number }) {
  return <Card className="group overflow-hidden border-border/70 bg-card/80 transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg"><div className="relative h-[112px] overflow-hidden sm:h-[124px]"><img src={visualFor(project.title, 'Project', index + 100)} alt={`${project.title} visual`} className="h-full w-full object-cover" loading="lazy" decoding="async" /><Badge className="absolute left-3 top-3 border-0 bg-emerald-500/90 text-white">Free / $0 start</Badge></div><CardContent className="p-5"><h3 className="line-clamp-2 text-base font-semibold leading-5">{project.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-muted-foreground">{project.description}</p><div className="mt-3 flex min-h-6 flex-wrap gap-1.5">{project.tags.slice(0, 3).map((tag) => <Badge key={tag} variant="secondary" className="text-[10px]">{tag}</Badge>)}</div><p className="mt-3 text-xs font-medium text-foreground/80">Best for: {project.bestFor}</p><Button asChild variant="outline" size="sm" className="mt-4 h-9 w-full"><a href={project.url} target="_blank" rel="noopener noreferrer">Start Project <ExternalLink className="ml-2 h-3.5 w-3.5" /></a></Button></CardContent></Card>;
}
...
export default function EnhancedResourceHub() {
  ...
  const resources = useMemo(() => curatedResources.filter((resource) => { if (section === 'projects' || section === 'community' || section === 'courses') return false; if (section !== 'all' && section !== 'featured' && resource.category !== section) return false; if (section === 'featured' && !resource.featured) return false; return matches([resource.title, resource.description, resource.category, ...resource.tags]); }), [q, section]);
  ...
  return <><Header showSidebarTrigger /><main className="mx-auto w-full max-w-[1500px] px-4 py-6 md:px-8 md:py-8"><div className="space-y-7">
    <section className="relative min-h-[230px] overflow-hidden rounded-3xl border border-border/80 bg-card px-6 py-7 sm:min-h-[250px] sm:px-8 sm:py-8 lg:px-10"><img src={visualFor('Resource Hub', 'AI Learning', 77)} alt="" aria-hidden="true" className="absolute right-0 top-0 h-full w-[42%] object-cover opacity-20" /><div className="absolute inset-0 bg-gradient-to-r from-background via-background/95 to-background/60" /><div className="relative max-w-3xl space-y-4"><div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><span className="text-[11px] font-semibold uppercase tracking-[0.2em]">Resource Hub</span></div><h1 className="text-3xl font-semibold tracking-tight sm:text-4xl lg:text-5xl">Learn. Build. Connect.</h1><p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">First-party learning platforms, high-value $0-start project stacks and communities worth joining — with only a few genuinely important flagship courses.</p><div className="relative max-w-2xl"><Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search platforms, projects, communities..." className="h-11 rounded-xl border-border bg-background/90 pl-11" /></div></div></section>
