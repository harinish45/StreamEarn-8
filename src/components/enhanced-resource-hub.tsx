...
import { curatedResources, featuredCourses, type CuratedResource } from '@/lib/resource-hub-curated';

type FilterId = 'all' | CuratedResource['category'] | 'featured' | 'courses';
      ...
      <div className="min-w-0 flex-1">
        <CardHeader className="space-y-2 px-4 pb-2 pt-3 sm:px-5">
          <div className="flex items-start justify-between gap-3">
            <CardTitle className="min-w-0 text-[15px] font-semibold leading-5" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
              {resource.title}
            </CardTitle>
            <button aria-label={`Favorite ${resource.title}`} onClick={() => toggle(resource.id)} className="shrink-0 rounded-md p-1.5 text-muted-foreground transition hover:text-primary">
              <Heart className={`h-4 w-4 ${favorite ? 'fill-current text-primary' : ''}`} />
            </button>
          </div>
          <p className="text-xs leading-5 text-muted-foreground" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {resource.description}
          </p>
        </CardHeader>
        <CardContent className="px-4 pb-4 pt-1 sm:px-5">
          <div className="mb-3 flex min-h-6 flex-wrap gap-1.5">
            {resource.tags.slice(0, 3).map((tag) => <Badge key={tag} variant="secondary" className="text-[10px] font-medium">{tag}</Badge>)}
          </div>
          <Button asChild variant="outline" size="sm" className="h-9 w-full rounded-lg">
  ...
  return (
    <Card className="overflow-hidden border-primary/20 bg-card/80">
      <div className="h-[108px] overflow-hidden">
        <img src={visualFor(course.title, 'AI Learning', index + 40)} alt={`${course.title} visual`} className="h-full w-full object-cover" loading="lazy" decoding="async" />
      </div>
      <CardContent className="p-5">
        <Badge variant="outline" className="mb-3">Flagship course</Badge>
        <h3 className="text-base font-semibold leading-5">{course.title}</h3>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">{course.description}</p>
        <Button asChild variant="outline" size="sm" className="mt-4 w-full"><a href={course.url} target="_blank" rel="noopener noreferrer">View course <ExternalLink className="ml-2 h-3.5 w-3.5" /></a></Button>
      </CardContent>
    </Card>
  );
}
...
export default function EnhancedResourceHub() {
  ...
  const resources = useMemo(() => curatedResources.filter((resource) => {
    if (section === 'courses') return false;
    if (section !== 'all' && section !== 'featured' && resource.category !== section) return false;
    if (section === 'featured' && !resource.featured) return false;
    if (!q) return true;
    return [resource.title, resource.description, resource.category, ...resource.tags].join(' ').toLowerCase().includes(q);
  }), [q, section]);

  const courses = useMemo(() => featuredCourses.filter((course) => !q || [course.title, course.provider, course.description].join(' ').toLowerCase().includes(q)), [q]);
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-[1500px] px-4 py-6 md:px-8 md:py-8">
        <div className="space-y-7">
          <section className="relative min-h-[250px] overflow-hidden rounded-3xl border border-border/80 bg-card px-6 py-7 sm:min-h-[270px] sm:px-8 sm:py-8 lg:px-10">
            <img src={visualFor('Resource Hub', 'AI Learning', 77)} alt="" aria-hidden="true" className="absolute right-0 top-0 h-full w-[48%] object-cover opacity-25" />
            ...
            <div className="relative max-w-3xl space-y-4">
              ...
              <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl lg:text-5xl">Learn from the platforms that build the industry.</h1>
              <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">A focused library of first-party learning platforms from OpenAI, Anthropic, Google, Microsoft, AWS, NVIDIA and other leading organizations — with only a few genuinely important flagship courses.</p>
              <div className="relative max-w-2xl">
                <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search platforms, AI, cloud, security..." className="h-11 rounded-xl border-border bg-background/90 pl-11" />
              </div>
            </div>
          </section>

          <div className="flex gap-2 overflow-x-auto pb-1">
            {filters.map(({ id, label, icon: Icon }) => <Button key={id} variant={section === id ? 'default' : 'outline'} className="shrink-0 rounded-full" onClick={() => setSection(id)}><Icon className="mr-2 h-4 w-4" />{label}</Button>)}
          </div>

          {section === 'courses' ? (
            <section className="space-y-4">
              <div><h2 className="text-xl font-semibold">Flagship courses only</h2><p className="text-sm text-muted-foreground">Kept separate so the platform directory stays clean.</p></div>
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{courses.map((course, index) => <CourseCard key={course.id} course={course} index={index} />)}</div>
            </section>
          ) : (
            <>
              <div className="flex items-center justify-between gap-3 text-sm text-muted-foreground">
                <span>{resources.length} verified platforms</span>
                <div className="flex gap-1 rounded-lg border bg-card p-1"><Button size="sm" variant={view === 'grid' ? 'default' : 'ghost'} onClick={() => setView('grid')} aria-label="Grid view"><Grid2X2 className="h-4 w-4" /></Button><Button size="sm" variant={view === 'list' ? 'default' : 'ghost'} onClick={() => setView('list')} aria-label="List view"><List className="h-4 w-4" /></Button></div>
              </div>
              <div className={view === 'grid' ? 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'space-y-3'}>
                {resources.map((resource, index) => <ResourceCard key={resource.id} resource={resource} index={index} list={view === 'list'} favorite={favorites.has(resource.id)} toggle={toggle} />)}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t pt-5">
            <span className="text-sm text-muted-foreground">Need the broader ecosystem?</span>
            <Button asChild variant="outline" size="sm"><Link href="/ai-tools"><Sparkles className="mr-2 h-4 w-4" />AI Directory</Link></Button>
            <Button asChild variant="outline" size="sm"><Link href="/news">AI News</Link></Button>
          </div>
        </div>
      </main>
    </>
