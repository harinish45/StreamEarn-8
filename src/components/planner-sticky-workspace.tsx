  ...
  try{
    ...
    const v2=(parsed:any)=>{
      ...
      const notes=parsed.filter(Boolean).map((n:any,i:number)=>({
        ...
        color:validPalette(n.color),
        style:typeof n.style==='number'?(['plain','lined','grid','legal','stripe'][Math.max(0,Math.min(4,n.style-1))] as PaperStyle):validPaper(n.style),
        x:Number.isFinite(n.x)?n.x:24+(i%4)*285,
        y:Number.isFinite(n.y)?n.y:24+(Math.floor(i/4)%5)*275,
...
function Sticky({note,patch,startDrag}:{note:Sticky;patch:(id:string,p:Partial<Sticky>)=>void;startDrag:(e:React.PointerEvent<HTMLDivElement>)=>void}){
  ...
  return <article onPointerDown={startDrag} className="absolute flex h-[250px] w-[250px] flex-col rounded-[3px] p-4 text-[#262019] shadow-[0_5px_12px_rgba(0,0,0,.4),0_25px_45px_-12px_rgba(0,0,0,.6)] transition-[transform,box-shadow] duration-150 hover:shadow-[0_8px_16px_rgba(0,0,0,.4),0_32px_55px_-12px_rgba(0,0,0,.72)]" style={{left:note.x,top:note.y,transform:`rotate(${note.rotation}deg)`,background:`linear-gradient(175deg,${p.p1},${p.p2})`,opacity:note.archived?.45:1}}>
    ...
    <div className="relative z-10 flex items-center justify-between gap-2"><label className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-[.12em] opacity-60"><input type="checkbox" checked={note.done} onChange={e=>patch(note.id,{done:e.target.checked})} className="h-3.5 w-3.5"/>Done</label><div className="flex items-center gap-1"><button onClick={()=>patch(note.id,{archived:!note.archived})} className="rounded-full bg-black/10 p-1 opacity-60 hover:opacity-100" title={note.archived?'Restore':'Archive'} aria-label={note.archived?'Restore note':'Archive note'}>{note.archived?<RotateCcw className="h-3 w-3"/>:<Archive className="h-3 w-3"/>}</button><button onClick={()=>patch(note.id,{color:nextPalette})} className="h-4 w-4 rounded border border-black/10" style={{background:colors[nextPalette].p1}} title="Next colour" aria-label="Change note colour"/><button onClick={()=>patch(note.id,{style:nextPaper})} className="rounded-full bg-black/10 p-1 text-[9px] opacity-60 hover:opacity-100" title="Next paper style" aria-label="Change note paper style">↗</button></div></div>
