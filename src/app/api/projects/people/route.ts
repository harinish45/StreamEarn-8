...
import { projectPersonCreateSchema } from '@/lib/api-validation';
...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(!parsed.success)return NextResponse.json({error:'Invalid collaborator',issues:parsed.error.issues.map(issue=>issue.message)},{status:400});
    const {projectId,name,role,organization,notes}=parsed.data;
    const {data:p}=await sb.from('projects').select('id').eq('id',projectId).eq('owner_id',user.id).maybeSingle();
    if(!p)return NextResponse.json({error:'Not found'},{status:404});
    const {data,error}=await sb.from('project_people').insert({project_id:projectId,owner_id:user.id,name,role,organization,notes}).select('*').single();
