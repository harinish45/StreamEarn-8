...
export async function POST(request: NextRequest) {
  ...
  if (!Number.isFinite(contentLength) || contentLength > 64 * 1024) {
    return NextResponse.json({ error: 'Request too large' }, {
      status: 413,
      headers: { 'Cache-Control': 'no-store' },
    });
  }
  ...
  try {
    ...
    if (body.publishedAt) {
      ...
      if (Number.isNaN(d.getTime())) {
        return NextResponse.json({ error: 'Invalid publication date' }, {
          status: 400,
          headers: { 'Cache-Control': 'no-store' },
        });
      }
    ...
    // Only the scheduled/cron writer uses the service-role key.
    ...
    if (error instanceof Error && error.name === 'ZodError') {
      return NextResponse.json({ error: 'Invalid scheduler item' }, {
        status: 400,
        headers: { 'Cache-Control': 'no-store' },
      });
    }
