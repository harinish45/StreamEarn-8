...
export default function DirectoryPage() {
  ...
  useEffect(() => {
    try {
      ...
      setLinks(curatedLinks.map((item) => ({
        ...
        visitCount: Number.isFinite(saved[item.id]?.visitCount)
          ? Math.max(0, Number(saved[item.id]?.visitCount))
          : 0,
      })));
    } catch {
      setLinks(curatedLinks);
    }
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return links.filter((link) => {
      ...
      const matchesFavorite = !favoritesOnly || link.isFavorite;
  ...
  function persist(next: ResourceLink[]) {
    ...
    try {
      const compact = Object.fromEntries(
        next.map((item) => [item.id, { isFavorite: item.isFavorite, visitCount: item.visitCount }]),
      );
      ...
      <SidebarInset className="min-w-0 bg-[#0d0c0a]">
        <main className="min-h-screen bg-[#0d0c0a] px-4 py-8 text-[#f1ece3] sm:px-8 lg:px-12">
          <section className="mx-auto max-w-7xl">
              ...
              <div className="flex flex-wrap items-end justify-between gap-5">
                <div>
                  ...
                  <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">
                    Top 50 resources, <em className="font-serif font-normal">organized.</em>
                  </h1>
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-[#9d978e]">
                    A focused collection of the 50 core resources across AI, development, cybersecurity, careers, cloud and platforms.
                  </p>
                </div>
                <div className="text-right text-xs text-[#777168]">
                  <strong className="text-2xl font-medium text-[#f1ece3]">{filtered.length}</strong>
                  <br />of 50
                </div>
              </div>
            </header>
            ...
            <div className="mb-6 flex flex-col gap-3 lg:flex-row">
              <label className="flex h-11 flex-1 items-center gap-3 rounded-xl border border-[#302c27] bg-[#151311] px-4 text-[#8e887f] focus-within:border-[#ff6b5a]">
                ...
                <input
                  ...
                  placeholder="Search resources, companies, skills..."
                  className="w-full bg-transparent text-sm text-[#f1ece3] outline-none placeholder:text-[#625d56]"
                />
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`rounded-xl border px-4 text-xs ${favoritesOnly ? 'border-[#ff6b5a] bg-[#211613] text-[#ff8b7d]' : 'border-[#302c27] bg-[#151311] text-[#9d978e]'}`}
                >
                  <Heart className="mr-2 inline h-3.5 w-3.5" />Favorites
                </button>
                <div className="flex rounded-xl border border-[#302c27] bg-[#151311] p-1">
                  <button aria-label="Grid view" onClick={() => setView('grid')} className={`rounded-lg p-2 ${view === 'grid' ? 'bg-[#29231f] text-[#f1ece3]' : 'text-[#777168]'}`}>
                    <Grid2X2 className="h-4 w-4" />
                  </button>
                  <button aria-label="List view" onClick={() => setView('list')} className={`rounded-lg p-2 ${view === 'list' ? 'bg-[#29231f] text-[#f1ece3]' : 'text-[#777168]'}`}>
            ...
            <div className="mb-7 flex gap-2 overflow-x-auto pb-1">
              {categories.map((item) => (
                <button
                  ...
                  className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-[11px] ${category === item ? 'border-[#ff6b5a] bg-[#211613] text-[#ff8b7d]' : 'border-[#302c27] text-[#817b73] hover:border-[#514a43]'}`}
                >
                  {item}
                </button>
              ))}
            ...
            {filtered.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[#302c27] px-6 py-16 text-center text-sm text-[#777168]">
