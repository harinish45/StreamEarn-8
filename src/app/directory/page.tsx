...
function curatedFor(primaryCategory: string, activeFilters: Set<string>) {
  let pool = activeLinks();
...
export default function DirectoryPage() {
  ...
  const [links, setLinks] = useState<ResourceLink[]>(() => curatedFor('All', new Set()));

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}') as Record<string, SavedState>;
      setLinks(curatedFor(primaryCategory, activeFilters).map((item) => ({
        ...item,
      ...
      setLinks(curatedFor(primaryCategory, activeFilters));
    }
  ...
  const availableCount = curatedFor(primaryCategory, activeFilters).length;
