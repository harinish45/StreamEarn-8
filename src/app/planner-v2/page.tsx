...
import { StickyWallPage } from '@/app/planner/sticky/page';

const KEY = 'streamearn-planner-v6';
type Tab = 'today'|'week'|'calendar'|'tasks'|'notes'|'recurring'|'database'|'progress'|'sticky';
...
function loadStore():Store { if(typeof window==='undefined') return fresh(); try { const raw=window.localStorage.getItem(KEY); return raw ? sanitize(JSON.parse(raw)) : fresh(); } catch { return fresh(); } }
...
export default function PlannerV2(){
  ...
  return <main className="min-h-screen bg-background"><div className="mx-auto max-w-[1280px] px-4 py-5 md:px-7 md:py-7">
    ...
    {tab==='sticky' && <StickyWallPage embedded />}
  </div></main>;
}
